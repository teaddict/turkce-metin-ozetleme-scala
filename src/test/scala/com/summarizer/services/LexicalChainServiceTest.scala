...
class LexicalChainServiceTest extends FunSpec {
  ...
  describe("build chains") {
    it("should build chains") {
      ...
      val chain1 = Chain(None, 0, 0.0, List((lexical1, "synonymy", "otomobil")))
      val chain2 = Chain(None, 0, 0.0, List((lexical1, "synonymy", "tekerlekli")))
      val chain3 = Chain(None, 0, 0.0, List((lexical2, "holo_member", "araç filosu")))
      val chain4 = Chain(None, 0, 0.0, List((lexical2, "holo_member", "toplu taşıma aracı")))
      val chain5 = Chain(None, 0, 0.0, List((lexical2, "synonymy", "motorlu")))
      chain5.addLexicalToChain(lexical3, "synonymy", "motorlu")
      val chain6 = Chain(None, 0, 0.0, List((lexical1, "hypernymy", "taşıt")))
      chain6.addLexicalToChain(lexical4, "hypernymy", "taşıt")
      val chain7 = Chain(None, 0, 0.0, List((lexical4, "synonymy", "çiftteker")))
